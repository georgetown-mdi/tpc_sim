...
dd[dd.keys()[0]]
for v in dd:
    dd[v] = [str(x) for x in dd[v].values()]
dd = dict([('rec-'+str(i)+'-org',dd[i]) for i in dd])

attr_mod_data_dictionary = {'last_name':[(0.1, surname_misspell_corruptor),
                                       (0.1, ocr_corruptor),
                                       (0.1, keyboard_corruptor),
                                       (0.7, phonetic_corruptor)],
                            'first_name':[(0.1, edit_corruptor2),
                                          (0.1, ocr_corruptor),
                                          (0.1, keyboard_corruptor),
                                          (0.7, phonetic_corruptor)],
                            'mailing_address_zipcode':[(0.8, keyboard_corruptor),
                                        (0.2, postcode_missing_val_corruptor)],
                            'mailing_address_city':[(0.1, edit_corruptor),
                                    (0.1, missing_val_corruptor),
                                    (0.4, keyboard_corruptor),
                                    (0.4, phonetic_corruptor)]}

attr_name_list = list(attr_mod_data_dictionary.keys())
...
#
moddd = test_data_corruptor.corrupt_records(dd)

outdd = [(v,moddd[v]) for v in moddd if v.split('-')[2]=='dup']

rec_dict[rec_dict.keys()[0]]
